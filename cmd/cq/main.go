...
func main() {
	...
	srv := cqserver.New(config.CollectorPort, config.QueryPort, config.ETCD.Prefix, config.ETCD.Endpoints, logger)
	logger.Info("starting server", "collector_port", config.CollectorPort, "query_port", config.QueryPort)
